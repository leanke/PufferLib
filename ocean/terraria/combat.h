...
static float do_mine_or_attack(Terraria* env, int focus_act) {
    ...
    if (focus_act == 0 || focus_act == 5) {
        // Default: facing direction; try body tile, fall back to head tile
    ...
    // Mining damage: hand=1, wood=1, stone=2, iron=4, gold=8
    static const uint8_t PICK_DMG[5] = {1, 1, 2, 4, 8};
...
static void do_place_block(Terraria* env) {
    Player* p = &env->player;
    int ptx = (int)p->px;
    int pty = (int)(p->py);
    int btx = ptx + (int)p->facing;
    int bty = pty;
    ...
    for (int i = 0; i < MAX_ENEMIES; i++) {
        ...
        switch (e->estate[i]) {
            ...
            case AI_ATTACK:
                ...
                if (e->eattack_cd[i] == 0 && p->iframes == 0) {
                    int raw_dmg = (int)ep->dmg;
                    ...
                    reward -= 0.2f; // penalty for taking damage
                }
                break;
        }
...
static void try_spawn_enemy(Terraria* env) {
    ...
    *rng ^= *rng << 13; *rng ^= *rng >> 17; *rng ^= *rng << 5;
    ...
    int sx = (int)spawn_x;
    float spawn_y = py; // fallback
    for (int ty = 0; ty < WORLD_H - 1; ty++) {
        if (TILE_TYPE(env->tiles[ty][sx]) != TILE_AIR &&
            TILE_TYPE(env->tiles[ty-1 < 0 ? 0 : ty-1][sx]) == TILE_AIR) {
            spawn_y = (float)ty - ENEMY_HH - 0.05f;
            break;
        }
    }

    // Pick enemy type: night → more varied, day → mostly slimes
    uint8_t etype;
    if (env->is_night) {
        etype = (uint8_t)((*rng >> 4) % ENEMY_COUNT);
    } else {
        etype = ENEMY_SLIME;
    }
    ...
    if (depth > 0.5f && (*rng & 3) == 0) etype = ENEMY_BAT;
    ...
    e->evy[slot]      = 0.0f;
    e->etype[slot]    = etype;
    e->ehp[slot]      = ENEMY_PROPS[etype].hp;
    e->estate[slot]   = AI_WANDER;
    e->eactive[slot]  = 1;
    e->estun[slot]    = 0;
    e->eattack_cd[slot] = 0;
    e->ewander_cd[slot] = 0;

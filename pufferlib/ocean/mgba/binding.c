...
#define MY_METHODS {"vec_get_positions", vec_get_positions, METH_VARARGS, "Get positions of all envs"}
...
static int my_log(PyObject *dict, Log *log) {
    ...
    assign_to_dict(dict, "x", log->x);
    assign_to_dict(dict, "y", log->y);
    assign_to_dict(dict, "map", log->z);
    return 0;
}

...
from .utils import data, ram_map
...
GLITCH = __file__.rstrip("pokegym.py") + "glitch/"
CUT_GRASS_SEQ = deque(
    [(0x52, 255, 1, 0, 1, 1), (0x52, 255, 1, 0, 1, 1), (0x52, 1, 1, 0, 1, 1)]
)
CUT_FAIL_SEQ = deque(
    [(-1, 255, 0, 0, 4, 1), (-1, 255, 0, 0, 1, 1), (-1, 255, 0, 0, 1, 1)]
)
CUT_SEQ = [
...
class Pokegym:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.reset_mem = 5
        self.countdown = 11
        self.inf_money = False
        self.save_video = False
        self.new_events = False
        self.thatguys_cnn = False
        self.db_path = Path(f"db/")
        ...
        self.safari = [
            ...
            224,
            225,
        ]  # 156 - safari gate2, 21: 'Safari Zone (Rest house 1)', 222: 'Safari Zone (Prize house)', 223: 'Safari Zone (Rest house 2)', 224: 'Safari Zone (Rest house 3)', 225: 'Safari Zone (Rest house 4)'
    ...
    def _get_obs(self):
        ...
        if self.extra_obs:
            return {
                "screen": self.render(),
                "fixed_window": self.get_fixed_window(
                    mmap, r, c, self.observation_space["screen"].shape
                ),
                "flute": np.array(
                    [ram_map.read_bit(self.game, 0xD76C, 0)], dtype=np.uint8
                ),
                "bike": np.array(
                    [ram_map.read_bit(self.game, 0xD75F, 0)], dtype=np.uint8
                ),
                "hideout": np.array(
                    [ram_map.read_bit(self.game, 0xD81B, 7)], dtype=np.uint8
                ),
                "tower": np.array(
                    [ram_map.read_bit(self.game, 0xD7E0, 7)], dtype=np.uint8
                ),
                "silphco": np.array(
                    [ram_map.read_bit(self.game, 0xD838, 7)], dtype=np.uint8
                ),
                "snorlax_12": np.array(
                    [ram_map.read_bit(self.game, 0xD7D8, 7)], dtype=np.uint8
                ),
                "snorlax_16": np.array(
                    [ram_map.read_bit(self.game, 0xD7E0, 1)], dtype=np.uint8
                ),
    ...
    def reset(
        ...
        if self.swarming:
            ...
            info |= {
                "state": {
                    tuple(sorted(list(self.required_events))): self.swarming_state()
                },  # .read()
    ...
    #     cursor.execute(
    #         "CREATE TABLE IF NOT EXISTS environment (env_id TEXT PRIMARY KEY,hm_count INTEGER,cut INTEGER)"
    #     )
    #     cursor.execute(
    #         "INSERT OR REPLACE INTO environment VALUES (?, ?, ?)",
    #         (str(self.env_id), self.hm_count, self.cut),
    #     )
    ...
    def expl_rew(self):
        ...
        elif not self.events.get_event(
            "EVENT_BEAT_ROCKET_HIDEOUT_GIOVANNI"
        ) and self.events.get_event("EVENT_FOUND_ROCKET_HIDEOUT"):
        ...
        ) and self.events.get_event("EVENT_BEAT_ROCKET_HIDEOUT_GIOVANNI"):
            if map_n in self.poketower:
                exploration_reward = 0.03 * len(self.seen_coords)
            else:
                exploration_reward = 0.02 * len(self.seen_coords)
        ...
        ) and self.events.get_event("EVENT_RESCUED_MR_FUJI"):
            if map_n == 149:
                exploration_reward = 0.03 * len(self.seen_coords)
            else:
                exploration_reward = 0.02 * len(self.seen_coords)
        ...
        ) and self.events.get_event("EVENT_GOT_POKE_FLUTE"):
            if map_n in self.silphco:
                exploration_reward = 0.03 * len(self.seen_coords)
            else:
                exploration_reward = 0.02 * len(self.seen_coords)
        elif not self.events.get_event("EVENT_GOT_HM03") or not self.events.get_event(
    ...
    def cut_rew(self):
        ...
        if ram_map.used_cut(self.game) == 61 and self.cut_counter >= 5:
            ram_map.write_mem(
                self.game, 0xCD4D, 00
            )  # address, byte to write resets tile check
            self.used_cut += 1
            self.cut_counter += 1
    ...
    def reward_sum(self):
        ...
        self.moves_obtained_reward = (
            sum(self.moves_obtained.values()) * self.reward_scale
        )
    ...
    def infos_dict(self):
        info = {
            "Data": {
                ...
                "beat_hideout": self.events.get_event(
                    "EVENT_BEAT_ROCKET_HIDEOUT_GIOVANNI"
                ),
        ...
        if self.swarming:
            ...
            if new_required_events:
                info |= {
                    "state": {
                        tuple(sorted(list(required_events))): self.swarming_state()
                    },
